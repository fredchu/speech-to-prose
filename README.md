...
> Turn a recording/video/YouTube into a **faithful Traditional-Chinese prose write-up** — not SRT subtitles, but with a per-paragraph approximate timestamp, a timestamped EPUB with cover by default, and automatic **bilingual (source + Chinese) output for non-Chinese audio**. A [Claude Code](https://claude.com/claude-code) skill.
> 把錄音／影片／YouTube 轉成**忠於原話的繁體中文整理短文**——不是 SRT 字幕，但每段開頭帶大概時間戳、預設輸出帶時間戳與封面的 EPUB，英文（非中文）影音則**自動產中英對照版**。一個 [Claude Code](https://claude.com/claude-code) skill。
...
Pipeline (Chinese audio): dual-ASR (Breeze + VibeVoice, cross-referenced for terms/English) → LLM prose integration (faithful fidelity) → noun verification pass → prose coverage gate → per-paragraph timestamp alignment → `.md` + EPUB (default on).
...
- `pandoc` — for the default EPUB output (skipped gracefully with `--no-epub`).
...
流程（中文影音）：雙路 ASR（Breeze + VibeVoice，交叉比對術語/英文）→ LLM 散文整理（faithful 模式）→ 名詞查證 pass → 散文品質 gate → 段落時間戳對齊 → `.md` + EPUB（預設開）。
...
- `pandoc`：預設 EPUB 輸出需要（`--no-epub` 時可省）。
